...
const Counter: React.FC = () => {
   ...
   return (
      <Stack tokens={{ childrenGap: 5, padding: 10 }} horizontal horizontalAlign="center">
         <DefaultButton iconProps={{ iconName: "Add" }} text={"10"} onClick={() => dispatch(incrementByAmount(10))} />
         <DefaultButton iconProps={{ iconName: "Add" }} onClick={() => dispatch(increment())} />
         <span style={{ fontSize: "20px", textAlign: "center", margin: "0 15px" }}>{count}</span>
         <DefaultButton iconProps={{ iconName: "Remove" }} onClick={() => dispatch(decrement())} />
         <DefaultButton iconProps={{ iconName: "Remove" }} text={"10"} onClick={() => dispatch(decrementByAmount(10))} />
      </Stack>
   );
};
